...
public class Board
{
    ...
    private void CreateBoard()
    {
        ...
        Count = boardSizeX * boardSizeY;
    }

    private List<Cell> bottomCells = new List<Cell>();
    private List<Cell> oldCells = new List<Cell>();

    public int Count;

    public bool IsBottomCell(Cell cell)
    {
        return bottomCells.Contains(cell);
    }
    ...
    {
        for (int i = bottomCells.Count - 1; i >= 0; i--)
        {
            if (!bottomCells[i].IsEmpty)
            {
                return bottomCells[i];
            }
        }
    ...
    public Cell FindSimilarCellInBoard(Cell cell)
    {     
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
    ...
    public Cell FindDifferentCellInBoard(Cell cell)
    {     
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                Cell c = m_cells[x, y];
                if (!c.IsSameType(cell) && !c.IsEmpty)
                {
                    return c;
                }
            }
    ...
    public Cell GetFirstInBoardThatNotEmpty()
    {     
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
    ...
    internal void Fill()
    {
        ...
        // NormalItem fix = new NormalItem();
        // fix.SetType(Utils.GetRandomNormalType());
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                NormalItem.eNormalType type = items[UnityEngine.Random.Range(0, items.Count)];
                items.Remove(type);
    ...
    public void MoveBackToOrigin(Cell bottomCell, Action callback)
    {
        int index = bottomCells.IndexOf(bottomCell);
        ...
        Cell oldCell = oldCells[index];
        Debug.LogWarning("oldCell: " + oldCell.name + ", index: " + index);
        oldCell.Assign(item);
        oldCells[index] = null;

        item.View.DOMove(oldCell.transform.position, 0.3f).OnComplete(() => { if (callback != null) callback(); });
    }
